...
namespace DAL.MongoDB.Notify
{
    public class NotifyMongoDAL
    {
        ...
        public async Task<NotifySummeryViewModel> getListNotify(int user_id_send)
        {
            try
            {
                var filter = Builders<ReceiverMessageViewModel>.Filter.Where(x => x.user_receiver_id == user_id_send && (x.seen_status == (Int16)SeenType.NOT_SEEN || x.seen_status == (Int16)SeenType.SEEN_ALL));
